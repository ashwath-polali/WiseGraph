...
export function ExportChartButtons({ studentName }: Props) {
  ...
  function inlineStyles(svg: SVGElement): SVGElement {
    ...
    elements.forEach((element) => {
      ...
      const originalElement = svg.querySelector(`[data-export-id="${htmlElement.getAttribute('data-export-id')}"]`) as HTMLElement;
  ...
  async function exportToPNG() {
    ...
    try {
      ...
      // Tag elements for style matching
      const elements = svg.querySelectorAll('*');
      elements.forEach((el, i) => {
        (el as HTMLElement).setAttribute('data-export-id', `el-${i}`);
      });
      ...
      // Remove data-export-id attributes
      svgClone.querySelectorAll('[data-export-id]').forEach(el => {
        el.removeAttribute('data-export-id');
      });
      ...
      img.onload = () => {
        ...
        canvas.toBlob((blob) => {
          ...
          link.download = `${studentName.replace(/\s+/g, '-')}-performance-profile.png`;
        ...
        // Cleanup
        elements.forEach(el => {
          (el as HTMLElement).removeAttribute('data-export-id');
        });
      };
  ...
  async function exportToPDF() {
    ...
    try {
      ...
      img.onload = () => {
        ...
        pdf.save(`${studentName.replace(/\s+/g, '-')}-performance-profile.pdf`);
